...
export const addManager = async (data: AddManagerRequest): Promise<{ success: boolean; message: string; manager: Manager }> => {
  try {
    ...
    const newManager: Manager = {
      id: result.manager?.id || result.manager?.managerId || result.manager?.employeeId || data.managerId,
      name: result.manager?.name || `${result.manager?.firstName || data.firstName} ${result.manager?.lastName || data.lastName}`,
      email: result.manager?.email || data.email,
      phone: result.manager?.phone || result.manager?.phoneNumber || data.phone,
      joinDate: result.manager?.joinDate || result.manager?.createdAt || result.manager?.hireDate || data.joinDate,
      status: (result.manager?.status || 'Active') as 'Active' | 'Under Review' | 'Frozen'
    }

    return {
      success: result.success !== false,
      message: result.message || `Manager Added Successfully!\n\nName: ${newManager.name}\nID: ${newManager.id}\nEmail: ${newManager.email}\n\nCredentials have been sent to the manager's email.`,
      manager: newManager
    }
  } catch (error) {
...
export const addEmployee = async (data: AddEmployeeRequest): Promise<{ success: boolean; message: string; employee: Employee }> => {
  try {
    ...
    const newEmployee: Employee = {
      id: result.employee?.id || result.employee?.employeeId || data.employeeId,
      name: result.employee?.name || `${result.employee?.firstName || data.firstName} ${result.employee?.lastName || data.lastName}`,
      specialization: result.employee?.specialization || result.employee?.specialty || result.employee?.skill || result.employee?.jobType || data.specialization,
      email: result.employee?.email || data.email,
      phone: result.employee?.phone || result.employee?.phoneNumber || data.phone,
      rating: typeof result.employee?.rating === 'number' ? result.employee.rating : 0,
      status: (result.employee?.status || 'Active') as 'Active' | 'On Leave' | 'Frozen'
    }
