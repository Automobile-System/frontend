    ...
    const loadTasks = async () => {
      ...
      try {
        ...
        const transformedTasks: Task[] = fetchedTasks.map((task: Record<string, unknown>) => ({
          ...
          vehicle: (task.vehicleRegNo as string) || "Unknown Vehicle",
          plateNumber: (task.vehicleRegNo as string) || "",
          deadline: task.deadline
            ? new Date(task.deadline as string).toLocaleString("en-US", {
                month: "2-digit",
                day: "2-digit",
                year: "numeric",
                hour: "2-digit",
            ...
            : "",
          customer: (task.customerName as string) || "",
          status: ((task.status as string)?.toLowerCase() || "not_started") as TaskStatus,
          timeElapsedSeconds: task.timeSpent ? Math.round((task.timeSpent as number) * 3600) : 0,
        }));
