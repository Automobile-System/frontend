    ...
    try {
      ...
      const tasks: Task[] = events.map((event: Record<string, unknown>) => ({
        ...
        time: event.startTime
          ? new Date(event.startTime as string).toLocaleTimeString("en-US", {
              hour: "numeric",
              minute: "2-digit",
              hour12: true,
            })
          : "09:00 AM",
        deadline: (event.startTime as string) || (event.endTime as string) || weekStart.toISOString(),
        type: "normal" as const, // Calendar events don't have priority in response
        vehicle: (event.vehicleRegNo as string) || "",
